...
contract FatCatCoin is ERC20, ERC20Burnable, ERC20Permit, AccessControl {
    ...
    constructor(address defaultAdmin, address _mintWallet)
    ...
    // Function to unfreeze the tokens and transfer them to the mint wallet
    function unfreezeTokens() external {
        require(block.timestamp >= freezeReleaseTime, "Freeze period has not ended yet");
        require(balanceOf(address(this)) >= frozenTokens, "No frozen tokens to unfreeze");

        // Transfer the frozen tokens to the mint wallet
        _transfer(address(this), mintWallet, frozenTokens);

        // Optionally, set frozenTokens to zero if you want to prevent any further transfers of these tokens
        frozenTokens = 0;
    }
}